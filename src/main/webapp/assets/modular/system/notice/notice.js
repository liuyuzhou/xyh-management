layui.use(['layer', 'form', 'table', 'admin', 'ax'], function () {
    ...
    Notice.initColumn = function () {
        return [[
            {type: 'checkbox'},
            {field: 'activateId', hide: true, sort: true, title: 'id'},
            {field: 'title', sort: true, title: '标题'},
            {field: 'address', sort: true, title: '地址'},
            {field: 'meetingPlace', sort: true, title: '会议地址'},
            {field: 'actTime', sort: true, title: '活动开始时间'},
            {field: 'endTime', sort: true, title: '活动结束时间'},
            {field: 'content', sort: true, title: '内容'},
            {field: 'participateNum', sort: true, title: '参加人数'},
            {field: 'cost', sort: true, title: '费用'},
            {field: 'actCreator', sort: true, title: '创建人'},
    ...
    Notice.onEditNotice = function (data) {
        ...
        top.layui.admin.open({
            type: 2,
            title: '通知详情',
            content: Feng.ctxPath + '/notice/notice_update/' + data.activateId,
            end: function () {
                admin.getTempData('formOk') && table.reload(Notice.tableId);
            }
        });
    ...
    Notice.onDeleteNotice = function (data) {
        var operation = function () {
            ...
            ajax.set("activateId", data.activateId);
            ajax.start();
        };
